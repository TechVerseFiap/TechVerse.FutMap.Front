			...
			>
				<div className="px-6 pb-4">
					<div className="pt-2 relative">
						<div className={`flex items-center gap-4 transition-all duration-700 ease-in-out ${isExpanded ? "flex-col items-start" : "flex-row items-center"}`}>
							<div
								className={`transition-all duration-700 ease-in-out 
									${isExpanded
										? "w-full h-32 mb-4"
										: "w-18 h-18 flex-shrink-0"
							...
							>
								{placeImgUrl ? (
									<img
										...
										className={`object-cover rounded-lg transition-all duration-700 ease-in-out ${isExpanded ? "w-full h-32" : "w-18 h-18"
											}`}
									/>
								) : (
									<div className={`
							...
							<div className={`flex flex-col justify-center transition-all duration-700 ease-in-out ${isExpanded ? "w-full" : "flex-1 min-h-[72px]"}`}>
								...
								<div className="flex items-center gap-6 mb-2 py-2">
									<div className="flex items-center leading-none">
										<Rating
											...
											emptySymbol={<TrophyIconEmpty className="w-5 h-5 mx-0.5" />}
											fullSymbol={<TrophyIconFull className="w-5 h-5 mx-0.5" />}
											readonly={true}
											fractions={1}
										/>
									</div>
									...
									{isExpanded
										? (<span
											className={`text-xs text-gray-600 transition-all duration-300 ${isExpanded
										...
										: isVerified
											? (
												<div
													className={`w-6 h-6 bg-blue-200 rounded-full flex items-center justify-center transition-all duration-300 ${!isExpanded
						...
						>
							<div className="space-y-3 mb-6">
								...
								<div className="flex items-center gap-3 text-gray-700">
									...
									<span className="text-sm">Idades {minAge}-{maxAge} • Todos os níveis</span>
								</div>
							</div>
						</div>
					</div>
						...
						<div className="grid grid-cols-4 gap-1 mb-2 mt-4">
							{[
								...
								<div
									key={index}
									className={`
											w-fulltransition-all duration-500 ease-in-out ${button.delay}
											${isExpanded
